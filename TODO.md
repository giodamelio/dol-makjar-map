...
- [ ] **HTML popups on click** — Click a marker to show a positioned HTML popup with title, description, and links to wiki pages. Real DOM, not canvas.
...
- [ ] **URL hash state** — Store selected place/district in `window.location.hash` so links can point directly to a location (e.g. `#place/15` or `#district/rookery`).
